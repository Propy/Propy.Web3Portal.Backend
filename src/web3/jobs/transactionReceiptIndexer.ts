...
export const fetchBlockInfoBatchRetryOnFailure = async (blockNumberBatch : string[], network: string, rawLatestBlockNumberWithinRangeLimit: string[], retryCount: number = 0) => {
  ...
  if(url) {
    ...
    try {
      ...
      retryCount++;
      if(retryCount < 10) {
        createErrorLog(`error fetching block info (transactionReceiptIndexer) at ${Math.floor(new Date().getTime() / 1000)}, retry #${retryCount}...`, JSON.stringify(e));
        await sleep(2000 + Math.floor(Math.random() * 5000) * retryCount);
        return await fetchBlockInfoBatchRetryOnFailure(blockNumberBatch, network, rawLatestBlockNumberWithinRangeLimit, retryCount);
      } else {
        createErrorLog(`retries failed, error fetching block (transactionReceiptIndexer) info at ${Math.floor(new Date().getTime() / 1000)}`, JSON.stringify(e));
      }
      return [];
    }
  }
...
export const transactionReceiptIndexer = async (
  ...
  createLog({"transactionReceiptIndexer transactionHashesCount:": transactionHashesCount})
  ...
  if(transactionHashesCount > 0) {
    ...
    for await(let batch of batches) {
      ...
      // log batch status
      createLog(`transactionReceiptIndexer fetching batch ${currentBatch} of ${batches.length} for ${meta}`);
      ...
      // log batch status
      createLog(`transactionReceiptIndexer fetched batch ${currentBatch} of ${batches.length} (${new Date().getTime() - startTime}ms)`);

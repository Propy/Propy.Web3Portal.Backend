const { ONFT_RECEIVED_EVENT_TABLE, EVM_TRANSACTION_TABLE, NETWORK_TABLE } = require("../tables");

exports.up = (knex) => knex.schema.createTable(ONFT_RECEIVED_EVENT_TABLE, table => {
  table.increments();
  ...
  .onDelete('CASCADE')
  .notNullable();
  table.string("src_eid").index().notNullable();
  table.string("token_id").index().notNullable();
  table.string("to_address").index().notNullable();
...
exports.down = knex => knex.schema.dropTable(ONFT_RECEIVED_EVENT_TABLE);
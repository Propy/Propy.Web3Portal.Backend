import { ONFTSentEventModel } from "../models";
...
class ONFTSentEventRepository extends BaseRepository {
    getModel() {
      return ONFTSentEventModel
    }
    ...
    async paginate(
      ...
      const results = await this.model.query().where(function (this: QueryBuilder<ONFTSentEventModel>) {
        if (contractAddress) {
    ...
    async clearRecordsByContractAddressAboveOrEqualToBlockNumber(network: string, contractAddress: string, blockNumber: number) {
      return await this.model.query().where(function (this: QueryBuilder<ONFTSentEventModel>) {
    ...
    async findEventByEventFingerprint(eventFingerprint: string) {
      ...
      const result = await this.model.query().where(function (this: QueryBuilder<ONFTSentEventModel>) {
...
export default new ONFTSentEventRepository()
